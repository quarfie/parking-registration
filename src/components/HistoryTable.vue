    ...
    <table class="w-full text-left">
      ...
      <tbody>
        <tr v-for="(item, idx) in items" :key="idx" class="border-b last:border-none text-sm">
          ...
          <td class="py-2 px-2">{{ item.Make }}</td>
          <td class="py-2 px-2">{{ item.Model }}</td>
        ...
        <tr v-if="!items?.length">
          <td colspan="5" class="text-center text-gray-400 text-sm py-6">No registrations yet.</td>
        </tr>
      </tbody>
    </table>
  </div>
